...
export class AdminLayoutComponent implements OnDestroy {
  ...
  private checkFirstLogin(): void {
    ...
    if (!slug) return;
    const key = `onboarding_seen_${slug}`;
    if (typeof localStorage !== 'undefined' && !localStorage.getItem(key)) {
      localStorage.setItem(key, '1');
      // Only redirect if not already on the onboarding page
      const currentUrl = this.router.url;
      if (!currentUrl.includes('/onboarding')) {
        this.router.navigateByUrl(this.tenantService.adminUrl() + '/onboarding');
      }
    }
  }
  ...
  readonly visibleNavItems = computed(() => {
    ...
    const items: NavItem[] = [
      ...
      { i18nKey: 'admin.nav.audit', icon: this.icon('audit'), route: base + '/audit', permission: 'settings.edit' },
